 ...
 * Props: theme, store, products
 */
export default function CatalogPreview({ theme = {}, store = {}, products = [] }) {
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div className="px-3 py-2.5 shadow-sm" style={{ backgroundColor: headerBg }}>
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0" style={{ textAlign: titleAlign }}>
            <p className="font-bold leading-tight truncate"
              style={{ fontSize: storeTitleSize, color: headerTxt, ...getWordArtStyle(theme) }}>
              {store.name || 'Mi Tienda'}
            </p>
            {theme.subtitle && (
              <p className="truncate" style={{ fontSize: 10, color: headerTxt + 'bb', marginTop: 1 }}>{theme.subtitle}</p>
            )}
          </div>
        </div>
      </div>
                ...
                return (
                  <div key={p.id} className="rounded-xl overflow-hidden flex flex-col"
                    ...
                    <div style={{ padding: compact ? '5px' : '7px' }} className="flex flex-col flex-1 gap-0.5">
                      <p className="font-bold leading-snug line-clamp-1" style={titleStyle}>{p.name}</p>
                      {cs.rating > 0 && (
                        <div className="flex">
